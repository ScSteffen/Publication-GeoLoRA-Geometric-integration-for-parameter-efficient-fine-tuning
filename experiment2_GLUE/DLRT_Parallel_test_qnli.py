...
def tokenize_function(examples):
    return tokenizer(
        ...
        truncation=True,
        max_length=512,
    )  # https://github.com/QingruZhang/AdaLoRA/blob/d10f5ebee16c478fa2f41a44a237b38e8c9b0338/NLU/scripts/run_debertav3_qnli.sh#L12
...
def init_wandb(run_id):
    for attempt in range(3):  # Retry up to 3 times
        try:
            wandb.init(project="qnli", name=f"parallel_run_{run_id}", reinit=True)
            return
...
for run in range(5):
    ...
    training_args = TrainingArguments(
        output_dir="./results/qnli/parallel",
        ...
        num_train_epochs=10,  # https://github.com/QingruZhang/AdaLoRA/blob/d10f5ebee16c478fa2f41a44a237b38e8c9b0338/NLU/scripts/run_debertav3_qnli.sh#L14
    ...
    with open(
        f"results/qnli/run_parallel_stsb_layer_ranks_run_{run + 1}.csv",
        mode="w",
        newline="",
    ) as file:
    ...
    print(
        f"CSV file 'results/qnli/run_parallel_stsb_layer_ranks_run_{run + 1}.csv' created successfully."
    )
    # Save the best model
