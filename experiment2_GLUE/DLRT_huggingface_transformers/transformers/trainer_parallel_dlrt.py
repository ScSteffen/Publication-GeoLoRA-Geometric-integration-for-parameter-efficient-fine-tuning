...
class DLRTTrainerParallel:
    ...
    def __init__(
        ...
        self.coefficient_steps = coefficient_steps
        self.dlrt_lora_layers = dlrt_lora_layers
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = (
                DLRTTrainerParallel.get_optimizer_cls_and_kwargs(self.args, opt_model)
            )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # print("New lora step using learning rate:", learning_rate)
                    for lora_module in self.dlrt_lora_layers:
                        # Invoke update step for parallel integrator
                        if step % (self.coefficient_steps + 1) == 0 and step > 2:
                            # print(step)
                            lora_module.step(
                                self.lr_scheduler.get_last_lr()[-1], self.weight_decay
                            )
                            # print(lora_module.r)

                        else:
                            lora_module.basis_grad_zero()
                            # print(lora_module.lora_U.grad[:, : lora_module.r])
                        if (
                            step % (self.coefficient_steps + 1)
                            == self.coefficient_steps
                        ):
                            for lora_module in self.dlrt_lora_layers:
                    ...
                    self.optimizer.step()  # only on non-lora parameters
                    # for param_group in self.optimizer.param_groups:
                    #    for param in param_group["params"]:
                    #        print(
                    #            f"Parameter shape: {param.shape}, Requires Grad: {param.requires_grad}"
                    #        )
    ...
    def evaluation_loop(
        ...
        print([lora_module.r for lora_module in self.dlrt_lora_layers])
        args = self.args
