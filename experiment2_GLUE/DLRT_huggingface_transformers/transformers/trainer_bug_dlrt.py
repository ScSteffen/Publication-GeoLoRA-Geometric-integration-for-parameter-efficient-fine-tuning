    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = (
                DLRTTrainerBUG.get_optimizer_cls_and_kwargs(self.args, opt_model)
            )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # ------- Update Bug integrator ------- #
                    # print("New lora step using learning rate:", learning_rate)
                    # print(step)

                    if step % (self.coefficient_steps + 1) == 0:
                        for lora_module in self.dlrt_lora_layers:
                            # Invoke basis update step for BUG integrator
                            lora_module.step(
                        ...
                        for lora_module in self.dlrt_lora_layers:
                            # Invoke coefficient update step for BUG integrator
                            lora_module.step(
                                self.lr_scheduler.get_last_lr()[-1],
                                dlrt_step="coefficients",
                                weight_decay=self.weight_decay,
                            )
                    if step % (self.coefficient_steps + 1) == self.coefficient_steps:
                        for lora_module in self.dlrt_lora_layers:
                            lora_module.step(
                                self.lr_scheduler.get_last_lr()[-1],
                                dlrt_step="truncate",
                            ...
                            print(lora_module.r)
                    # ------- Update Bug integrator ------- #
                    self.optimizer.step()  # update everything but the basis (including S Step)
