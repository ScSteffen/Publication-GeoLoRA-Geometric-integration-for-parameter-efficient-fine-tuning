...
def init_wandb(run_id):
    for attempt in range(3):  # Retry up to 3 times
        try:
            wandb.init(project="mnli", name=f"adalora_run_{run_id}", reinit=True)
            return
...
for run in range(5):
    ...
    with open(
        f"run_adalora_mnli_layer_ranks_run_{run + 1}.csv", mode="w", newline=""
    ) as file:
    ...
    print(
        f"CSV file 'run_adalora_mnli_layer_ranks_run_{run + 1}.csv' created successfully."
    )
