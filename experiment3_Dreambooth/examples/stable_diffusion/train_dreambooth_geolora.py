...
from peft.tuners.pdlrt.custom_peft import get_custom_peft
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help=(" tau coefficient for compression"),
    )
    parser.add_argument(
        "--mixed_precision",
        type=str,
...
def main(args):
    ...
    print(f'\n NET CREATED \n')
    from peft.tuners.lora_dlrt.extract_weights import count_params_conv_pdlrt
    from peft.tuners.lora import LoraLayer
    from peft import DLRT_model
    print(f'args {args}')
    if args.adapter != "full":
        unet,llr_unet = get_custom_peft(unet,peft_module="LoRAParallelDLRT",
                                        target_layer_names=UNET_TARGET_MODULES,
                                        rank=args.unet_r,
                                        max_rank = 2*args.unet_r,
    ...
    low_rank_layers = llr_unet

    vae.requires_grad_(False)
    if not args.train_text_encoder:
        text_encoder.requires_grad_(False)
    elif args.train_text_encoder and args.adapter != "full":
        text_encoder,llr_te = get_custom_peft(text_encoder,peft_module="LoRAParallelDLRT",
                                        target_layer_names=TEXT_ENCODER_TARGET_MODULES,
                                        rank=args.te_r,
                                        max_rank = 2*args.te_r,
                                        alpha=args.te_alpha,
                                        lora_dropout=args.te_dropout,
                                        tau = args.tau
                                        )
        # print(text_encoder)
    low_rank_layers+=llr_te
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(*([text_encoder.parameters()]+[l.parameters() for l in low_rank_layers]))
    )
        ...
        with TorchTracemalloc() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    ##### PARALLEL UPDATE ##########
                    coefficient_steps = 1
                    for lora_module in low_rank_layers:
                        if step % (coefficient_steps + 1) == 0 and step > 2:
                                lora_module.step(
                                    lr_scheduler.get_last_lr()[-1], args.adam_weight_decay
                                )

                        else:
                            lora_module.basis_grad_zero()
                        if (
                            step % (coefficient_steps + 1)
                            == coefficient_steps
                        ):
                            for lora_module in low_rank_layers:
                                lora_module.Truncate()
                                lora_module.basis_grad_zero()
                                lora_module.coeff_grad_zero()

                    # ------- Update parallel integrator ------- #
                    # Optimizer step
                ...
                # print(f'end batch loss: {loss.detach().item()}')
            ...
            count_params_conv_pdlrt(low_rank_layers)
            for l in low_rank_layers:
                print(f'rank: {l.r},dims:{l.dims}')
            print('='*100)
